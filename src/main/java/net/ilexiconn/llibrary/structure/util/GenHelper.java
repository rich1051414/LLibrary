...
public class GenHelper
{
    ...
    public static boolean addItemToTileInventory(World world, ItemStack itemstack, BlockPos pos)
    {
        ...
        {
            System.err.println("[LLibrary] Tile Entity at " + pos.getX() + "/" + pos.getY() + "/" + pos.getZ() + " is " + (tile != null ? "not an IInventory" : "null"));
            return false;
        }
    ...
    {
        double minX = (double) pos.getX(), minZ = (double) pos.getZ(), maxX = minX, maxZ = minZ;
        switch (direction)
        {
            case NORTH: // frame facing NORTH
                minX += 0.25D;
                maxX += 0.75D;
                minZ += 0.5D;
                maxZ += 1.5D;
                break;
            case SOUTH: // frame facing SOUTH
                minX += 0.25D;
                maxX += 0.75D;
                minZ -= 0.5D;
                maxZ += 0.5D;
                break;
            case WEST: // frame facing WEST
                minX += 0.5D;
                maxX += 1.5D;
                minZ += 0.25D;
                maxZ += 0.75D;
                break;
            case EAST: // frame facing EAST
                minX -= 0.5D;
                maxX += 0.5D;
                minZ += 0.25D;
                maxZ += 0.75D;
        ...
        return AxisAlignedBB.fromBounds(minX, (double) pos.getY(), minZ, maxX, (double) pos.getY() + 1, maxZ);
    }
    ...
    public static int setHangingEntity(World world, ItemStack hanging, BlockPos pos)
    {
        ...
        {
            System.err.println("[LLibrary] Hanging entity has invalid metadata of " + world.getBlockMetadata(pos) + ". Valid values are 1,2,3,4");
            return -1;
        }
    ...
    public static boolean setPaintingArt(World world, String name, BlockPos pos, EnumFacing direction)
    {
        ...
        System.err.println("[LLibrary] No EntityPainting was found at " + pos.getX() + "/" + pos.getY() + "/" + pos.getZ());
        return false;
    }
        ...
        if (sign != null)
        {
            for (int i = 0; i < sign.signText.length && i < text.length; ++i)
            {
                ...
                else if (text[i].length() > 15)
                {
                    ...
                    sign.signText[i] = new ChatComponentText(text[i].substring(0, 15));
                }
                else
                    sign.signText[i] = new ChatComponentText(text[i]);
            }

            return true;
        }

        System.err.println("[LLibrary] No TileEntitySign was found at " + pos.getX() + "/" + pos.getY() + "/" + pos.getZ());
        return false;
    }
    ...
    public static boolean setSkullData(World world, String name, int type, int rot, BlockPos pos)
    {
        ...
        System.err.println("[LLibrary] No TileEntitySkull found at " + pos.getX() + "/" + pos.getY() + "/" + pos.getZ());
        return false;
    }
    ...
    public static int getMetadata(int rotations, Block block, int origMeta)
    {
        ...
        int bit8 = (meta & 8); // usually 'on' or 'off' flag, but also
                               // top/bottom for doors
        int bit9 = (meta >> 3); // used by pistons for something, can't remember
                                // what...
