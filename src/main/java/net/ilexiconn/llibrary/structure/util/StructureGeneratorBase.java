...
public abstract class StructureGeneratorBase extends WorldGenerator
{
    ...
    /**
     * Constructs the generator with the player's facing, the blockArray for the structure and the structure's facing
     */
     ...
     * @param customData1
     *            Custom data may be used to subtype events for given fakeID Returns the real id of the block to spawn in the world; must be <= 4095
     */
                ...
                for (int z = 0; z < blockArray[y][x].length; ++z)
                {
                    ...
                    switch (rotations)
                    {
                        ...
                        case 3: // Rotate structure 270 degrees clockwise
                            ...
                            break;
                        default:
                            System.err.println("[LLibrary] Error computing number of rotations.");
                            break;
                    }
    ...
    private void setBlockAt(World world, int fakeID, int realID, int meta, int customData1, int customData2, BlockPos pos)
    {
        ...
        if (realID >= 0 || world.isAirBlock(pos) || !world.getBlockState(pos).getBlock().getMaterial().blocksMovement())
        {
    ...
    private boolean removeBlockAt(World world, int fakeID, int realID, int x, int y, int z, int rotations)
    {
        ...
        {
            System.err.println("[LLibrary] Incorrect location for structure removal, aborting. Last block id checked: world " + worldBlock + ", real " + realID + ", fake " + fakeID);
            return false;
        }
    ...
    private void doPostGenProcessing(World world)
    {
        ...
        for (BlockData block : postGenBlocks)
        {
            ...
            System.out.println("[LLibrary] Post-gen processing for initial ID: " + fakeID + ", returned id from getRealID: " + realID);
            Block realBlock = Block.getBlockById(Math.abs(realID));
            if (realID >= 0 || world.isAirBlock(block.getPosX(), block.getPosY(), block.getPosZ()) || !world.getBlock(block.getPosX(), block.getPosY(), block.getPosZ()).getMaterial().blocksMovement())
            {
                ...
                if (world.getBlockMetadata(block.getPosX(), block.getPosY(), block.getPosZ()) != block.getMetaData())
                    System.err.println("[LLibrary] Mismatched metadata. Meta from world: " + world.getBlockMetadata(block.getPosX(), block.getPosY(), block.getPosZ()) + ", original: " + block.getMetaData());
